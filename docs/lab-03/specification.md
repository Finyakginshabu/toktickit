...
1. **Authentication & Session Lifecycle**:
   ...
   - Mandatory first-login password change for accounts flagged with initial passwords, blocking application navigation until resolved.
...
4. **Requester Continuity & Features**:
   - Regression preservation of Create Ticket, My Tickets, Ticket Detail, and Attachment management without the development selector.
...
* **FR-03 (Mandatory First-Login Password Change)**: The system shall detect users with `mustChangePassword = true` upon login and redirect them to a mandatory password change screen, blocking access to normal application screens until a valid new password is saved.
...
* **FR-06 (Requester Functionality Continuity)**: Requesters shall continue to create tickets, view My Tickets, view Ticket Detail, and manage attachments using their authenticated identity without client-supplied `requesterId`.
...
* **FR-08 (IT Staff Ticket Queue)**: The system shall provide IT Staff with a unified Ticket Queue displaying tickets across all requesters, supporting search (summary and ticket number), filtering (category, status, priority, ownership), sorting, and pagination.
...
| :--- | :--- | :--- |
| **BR-01** | Authentication | Only an active user (`isActive = true`) with valid matching credentials may authenticate. Invalid credentials or inactive accounts return a safe generic failure message (`Invalid email or password`). |
| **BR-02** | First-Login Password | A user marked with `mustChangePassword = true` cannot access normal application screens until a new password meeting complexity requirements is saved. |
...
| **BR-06** | Credential Hashing & Complexity | Passwords must be hashed using bcrypt (salt rounds $\ge 10$) and never stored in plaintext. New passwords must be at least 8 characters and contain at least one uppercase letter, one lowercase letter, and one number or special character. |
...
| **BR-11** | Email Uniqueness | Email addresses must be globally unique across all accounts. Creating or updating a user with an existing email must be rejected with HTTP 409 Conflict. |
...
| **BR-13** | Ticket Ownership Assignment | A ticket may have zero or one primary Ticket Owner. Ownership may only be assigned to active users with role `IT_STAFF` or `ADMINISTRATOR`. |
...
| **BR-15** | Append-Only Communications | Public Comments and Internal Notes are strictly append-only. Modification and deletion are prohibited. Entries must contain 1 to 2000 characters after whitespace trimming. Whitespace-only entries are rejected. |
...
| **BR-18** | Attachment Continuity | All Lab 2 attachment constraints remain active: max 5 active attachments per ticket, allowed types (`JPG`, `PNG`, `WEBP`, `PDF`), max size 5 MB, soft-removal with recorded reason, and blocked download of removed files returning HTTP 410 Gone. |
...
* **Requester Screens**:
  ...
  * Requester Ticket Detail includes a read-only header, Attachment section, Public Comments thread, and a prominent **"Problem Appears Resolved"** action button.
...
### 7.3. Migration & Seed Decisions
* **Database Migration**: Evolve `RequesterUser` table to `User` without data loss. Existing ticket relationships (`requesterId`) remain preserved. Seeded Requesters receive default initial passwords (e.g., `Password123!`) with `mustChangePassword = false` for existing test stability, while newly created initial accounts receive `mustChangePassword = true`.
* **Seed Data Profile**:
...
* **Requester Tickets**: `GET /api/tickets/my-tickets`, `POST /api/tickets`, `GET /api/tickets/:id`, `POST /api/tickets/:id/indicate-resolved`.
...
* **AC-02 (Mandatory First-Login Password Change)**:
  ...
  * **When** login succeeds,
  * **Then** the user is directed to the Change Password screen, and normal application routes remain inaccessible until a valid password is saved.
...
* **AC-08 (Requester Regression)**:
  * **Given** an authenticated Requester,
  * **When** creating a ticket or uploading attachments,
  * **Then** all Lab 2 validation, attachment limits (5 max), and soft-removal behaviors function correctly.
...
* **AC-10 (IT Staff Ticket Queue Retrieval)**:
  ...
  * **Then** all tickets across all requesters are displayed with working search, filters (category, priority, status, ownership), sorting, and pagination.
...
* **AC-12 (Reassign Ticket Ownership)**:
  ...
  * **When** an IT Staff member or Admin selects another active IT Staff member as owner,
  * **Then** ticket ownership is updated in the database.
* **AC-13 (IT Priority Modification)**:
  * **Given** an active ticket,
  * **When** IT Staff modifies the IT Priority,
...
* **AC-16 (Public Comments Thread)**:
  ...
  * **Then** it is saved and visible to all authorized parties.
...
- [ ] **Backend Authorization**: All endpoints enforce server-side RBAC and ownership verification. Passwords securely hashed with bcrypt. Safe error status codes (401, 403, 404, 409) returned.
- [ ] **Frontend Screens**:
...
5. **Separation of Admin & IT Staff**: Administrators focus exclusively on account management (`/api/admin/users`) and do not appear in the operational queue unless explicitly assigned as ticket owners.
